...
#define DEBUG_SHOW_DEBUG    1
        ...
        #define LOGF(format, ...)   __d_debug( DEBUG_FATAL,     LOG_TAG, _gettid(), KRED, format "\n", ##__VA_ARGS__ ); abort()
        #define LOGE(format, ...)   __d_debug( DEBUG_ERROR,     LOG_TAG, _gettid(), KRED, format "\n", ##__VA_ARGS__ )
        #define LOGW(format, ...)   __d_debug( DEBUG_WARNING,   LOG_TAG, _gettid(), KBRN, format "\n", ##__VA_ARGS__ )
        #define LOGI(format, ...)   __d_debug( DEBUG_INFO,      LOG_TAG, _gettid(), KGRN, format "\n", ##__VA_ARGS__ )
        #define LOGD(format, ...)   __d_debug( DEBUG_DEBUG,     LOG_TAG, _gettid(), KBLU, format "\n", ##__VA_ARGS__ )
        #define LOGV(format, ...)   __d_debug( DEBUG_VERBOSE,   LOG_TAG, _gettid(), KCYN, format "\n", ##__VA_ARGS__ )
