...
static int hloop_process_ios(hloop_t* loop, int timeout) {
    ...
    if (nevents < 0) {
        LOGD("poll_events error=%d", -nevents);
    }
...
static void hloop_stat_timer_cb(htimer_t* timer) {
    ...
    LOGD("[loop] pid=%ld tid=%ld uptime=%lluus cnt=%llu nactives=%u nios=%u ntimers=%u nidles=%u",
