    ...
    if (remain) {
        ...
        if (io->readbuf.tail == io->readbuf.len) {
            if (io->readbuf.len >= setting->package_max_length) {
                LOGE("recv package over %d bytes!", (int)setting->package_max_length);
...
int hio_unpack_by_length_field(hio_t* io, void* buf, int readbytes)
{
    ...
    if (setting->predealdata) {
        setting->predealdata(io, buf, readbytes);
    }
    const unsigned char* p = sp;
    int remain = ep - p;
    int handled = 0;
    ...
    while (remain >= setting->body_offset) {
        ...
        switch (setting->length_field_coding) {
        case BIG_ENDIAN:
    ...
    if (remain) {
        ...
        if (package_len > io->readbuf.len) {
            if (package_len > setting->package_max_length) {
                LOGE("package length over %d bytes!", (int)setting->package_max_length);
                io->error = ERR_OVER_LIMIT;
                hio_close(io);
                return -1;
            }
