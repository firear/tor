...
accept_error:
    LOGE("listenfd=%d accept error: %s:%d", io->fd, socket_strerror(io->error), io->error);
    hio_close(io);
}
...
connect_error:
    LOGW("connfd=%d connect error: %s:%d", io->fd, socket_strerror(io->error), io->error);
    hio_close(io);
}
...
int hio_read (hio_t* io) {
    if (io->closed) {
        LOGE("hio_read called but fd[%d] already closed!", io->fd);
        return -1;
    }
...
int hio_write (hio_t* io, const void* buf, size_t len) {
    if (io->closed) {
        LOGE("hio_write called but fd[%d] already closed!", io->fd);
        return -1;
    }
    int nwrite = 0, err = 0;
    hrecursive_mutex_lock(&io->write_mutex);
...
enqueue:
    ...
    if (nwrite < len) {
        if (io->write_bufsize + len - nwrite > io->max_write_bufsize) {
            LOGE("write bufsize > %u, close it!", io->max_write_bufsize);
        ...
        if (io->write_bufsize > WRITE_BUFSIZE_HIGH_WATER) {
            LOGW("write len=%d enqueue %u, bufsize=%u over high water %u",
...
int hio_close (hio_t* io) {
    ...
    if (!write_queue_empty(&io->write_queue) && io->error == 0 && io->close == 0) {
        io->close = 1;
        hrecursive_mutex_unlock(&io->write_mutex);
        LOGW("write_queue not empty, close later.");
