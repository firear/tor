...
static inline hthread_t hthread_create(hthread_routine fn, void* userdata) {
    ...
    return (HANDLE)_beginthreadex(NULL, 0, fn, userdata, 0, NULL); //instead of win32 CreateThread witch maybe cause tiddata memory leak
}
...
static inline int hthread_detach(hthread_t th) {
    CloseHandle(th);
    return 0;
}
#else
